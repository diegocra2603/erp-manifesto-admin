/**
 * ============================================
 * API ENDPOINT - ACCOUNTING PERIOD BY ID & SUB-RESOURCES
 * ============================================
 * Handles: GET/PUT/DELETE /api/accountingperiod/:id
 *          POST /api/accountingperiod/:id/close
 */
...
async function proxyRequest(method: string, backendPath: string, authHeader: string, body?: unknown) {
  ...
  const response = await fetch(`${API_URL}/api/AccountingPeriod/${backendPath}`, {
    method,
    headers,
