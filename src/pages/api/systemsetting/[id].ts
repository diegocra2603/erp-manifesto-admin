/**
 * ============================================
 * API ENDPOINT - SYSTEM SETTING BY ID
 * ============================================
 * Server-side endpoint for updating a system setting by ID (JSON)
 */
...
export const PUT: APIRoute = async ({ params, request }) => {
  try {
    ...
    if (!id) {
      return new Response(
        JSON.stringify({
          ...
          status: 400,
          errors: {
            'Request.Invalid': ['ID de la configuración es requerido'],
          },
    ...
    const response = await fetch(`${API_URL}/api/SystemSetting/${id}`, {
      method: 'PUT',
      headers: {
    ...
    const setting: SystemSetting = data ?? {};
