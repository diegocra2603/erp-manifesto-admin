/**
 * ============================================
 * API ENDPOINT - PRODUCT BY ID & SUB-RESOURCES
 * ============================================
 * Handles: GET/PUT/DELETE /api/product/:id
 *          POST /api/product/:id/job-positions
 *          DELETE /api/product/:id/job-positions/:jpId
 */
...
async function proxyRequest(method: string, backendPath: string, authHeader: string, body?: unknown) {
  ...
  const response = await fetch(`${API_URL}/api/Product/${backendPath}`, {
    method,
    headers,
