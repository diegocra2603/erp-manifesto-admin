/**
 * ============================================
 * API ENDPOINT - USER BY ID
 * ============================================
 * Server-side endpoint for getting, updating, and deleting a user by ID (JSON)
 */
    ...
    if (!id) {
      return new Response(
        JSON.stringify({
          ...
          status: 400,
          errors: {
            'Request.Invalid': ['ID del usuario es requerido'],
          },
...
export const PUT: APIRoute = async ({ params, request }) => {
  try {
    ...
    // Safely parse JSON — backend may return empty body (204)
    let data = null;
    const contentType = response.headers.get('content-type');
    const text = await response.text();
    ...
    if (text && contentType?.includes('application/json')) {
      try {
        ...
        // not valid JSON, keep data as null
      }
    }
